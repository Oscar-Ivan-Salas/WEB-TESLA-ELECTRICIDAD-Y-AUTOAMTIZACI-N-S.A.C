/* ============================================================
   React Bits → JS puro
   SplashCursor: simulación de fluido WebGL que sigue el cursor
   Uso: window.initSplashCursor(containerDiv, { ...opts })
   ============================================================ */

(function () {
    'use strict';

    window.initSplashCursor = function (container, opts) {
        opts = opts || {};
        ...
        var config = {
            SIM_RESOLUTION: opts.SIM_RESOLUTION || 128,
            DYE_RESOLUTION: opts.DYE_RESOLUTION || 1024,
            CAPTURE_RESOLUTION: opts.CAPTURE_RESOLUTION || 512,
            DENSITY_DISSIPATION: opts.DENSITY_DISSIPATION || 3.5,
            VELOCITY_DISSIPATION: opts.VELOCITY_DISSIPATION || 2,
            PRESSURE: opts.PRESSURE || 0.1,
            PRESSURE_ITERATIONS: opts.PRESSURE_ITERATIONS || 20,
            CURL: opts.CURL || 3,
            SPLAT_RADIUS: opts.SPLAT_RADIUS || 0.2,
            SPLAT_FORCE: opts.SPLAT_FORCE || 6000,
            SHADING: opts.SHADING !== false,
            COLOR_UPDATE_SPEED: opts.COLOR_UPDATE_SPEED || 10,
            PAUSED: false,
            BACK_COLOR: opts.BACK_COLOR || { r: 0.5, g: 0, b: 0 },
            TRANSPARENT: opts.TRANSPARENT !== false,
            RAINBOW_MODE: !!opts.RAINBOW_MODE,
            COLOR: opts.COLOR || '#ff0000',
            COLORS: opts.COLORS || null,
            COLOR_INTENSITY: opts.COLOR_INTENSITY || 0.3
        };
    ...
    `;
      ...
      `
        ...
        function hexToRGB(hex) {
            ...
            var intensity = config.COLOR_INTENSITY || 0.15;
        ...
        function generateColor() {
            if (!config.RAINBOW_MODE) {
                if (config.COLORS && config.COLORS.length > 0) {
                    var brandHex = config.COLORS[Math.floor(Math.random() * config.COLORS.length)];
